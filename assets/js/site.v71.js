...
// Static image fallbacks for best work + case tiles
(() => {
  const attach = (img, fallback) => {
  ...
  document.querySelectorAll(".library-card__img").forEach(img => {
    attach(img, "assets/images/placeholders/thumb-generic.svg");
  });
  document.querySelectorAll(".case-thumb img").forEach(img => {
    attach(img, "assets/images/placeholders/thumb-case.svg");
  });
})();
...
(function(){
  ...
  function resolveThumbByType(type, kind){
    const key = (type || kind || "").toLowerCase();
    if (key.includes("video")) return "assets/images/placeholders/thumb-video.svg";
    if (key.includes("campaign")) return "assets/images/placeholders/thumb-campaign.svg";
    if (key.includes("resource")) return "assets/images/placeholders/thumb-resource.svg";
    if (key.includes("award")) return "assets/images/placeholders/thumb-award.svg";
    if (key.includes("press")) return "assets/images/placeholders/thumb-press.svg";
    if (key.includes("case")) return "assets/images/placeholders/thumb-case.svg";
    if (key.includes("blog")) return "assets/images/placeholders/thumb-blog.svg";
    if (key.includes("web")) return "assets/images/placeholders/thumb-web.svg";
    if (key.includes("social")) return "assets/images/placeholders/thumb-social.svg";
    return "assets/images/placeholders/thumb-generic.svg";
  }

  function attachImgFallback(img, fallback){
    if (!img || !fallback) return;
    img.addEventListener("error", ()=>{
      if (img.dataset.fallbackApplied) return;
      img.dataset.fallbackApplied = "true";
  ...
  function buildCard({title, eyebrow, desc, meta, href, thumb, tag, kind, type}){
    const a = el("a", "gcard", {href: href || "#", target: href ? "_blank" : "_self", rel: href ? "noopener noreferrer" : ""});
    a.dataset.kind = kind || "";
    const media = el("div", "gcard__media");
    const fallback = resolveThumbByType(type || kind || tag);
    const resolvedThumb = thumb || fallback;
    const img = el("img", "gcard__img", {src: resolvedThumb, alt: ""});
    img.loading = "lazy";
    attachImgFallback(img, fallback);
  ...
  function buildLibraryCard({title, description, url, thumb, tag, type}){
    ...
    const fallback = resolveThumbByType(type || tag);
    const resolvedThumb = thumb || fallback;
    const img = el("img", "library-card__img", {src: resolvedThumb, alt: ""});
    img.loading = "lazy";
    attachImgFallback(img, fallback);
    media.appendChild(img);
  ...
  function buildSocialCard({title, description, url, thumb, platform, tag}){
    ...
    const fallback = resolveSocialThumb(platform);
    const resolvedThumb = thumb || fallback;
  ...
  function applyProofWallFallbacks(){
    qsa(".proof-tile__img").forEach(img => {
      attachImgFallback(img, "assets/images/placeholders/thumb-campaign.svg");
    });
  }
