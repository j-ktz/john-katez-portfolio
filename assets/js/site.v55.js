...
(function(){
  ...
  function initCreativeGallery(){
    ...
    function render(){
      ...
      vids.forEach(v => {
        const thumb = v.video_id ? `https://i.ytimg.com/vi/${v.video_id}/hqdefault.jpg` : "";
        const meta = [v.platform, v.date, v.views ? `${formatViews(v.views)} views` : ""].filter(Boolean).join(" • ");
        const c = buildCard({
          title: v.title,
        ...
        c.addEventListener("click", (e)=>{ e.preventDefault(); openVideoModal(v); });
      ...
      const proof = state.links
        ...
        .filter(l => ["award","press","campaign","blog"].includes(l.type || ""))
