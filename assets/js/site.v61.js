(function () {
  ...
  if (statsWrap) {
    ...
    const ROTATE = [["Content CoE", "Center of Excellence leadership"], ["Governance system", "Approvals + escalation paths"], ["Exec storytelling", "Thought leadership activation"], ["10K+ interactions", "Single LinkedIn post"], ["37.5% engagement", "LinkedIn engagement rate"], ["67K+ views", "YouTube campaign highlight"], ["Awards pipeline", "Webby / Anthem / MarCom submissions"], ["Ops playbooks", "Toolkits + offboarding workflows"], ["Performance reporting", "Dashboards + insights"], ["Platform-native", "Repurposing + optimization"], ["Editorial planning", "Calendars + briefs"], ["Enablement & training", "Newsrooms + Lunch & Learns"]];
    ...
    if (!prefersReduced) {
      setInterval(() => {
        ...
        if (document.documentElement.classList.contains('mode--recruiter')) return;
...
(function(){
  ...
  function buildLibraryCard({title, description, url, thumb, tag, type}){
    const a = el("a", "library-card", {href: url || "#", target: url && /^https?:\/\//i.test(url) ? "_blank" : "_self", rel: url && /^https?:\/\//i.test(url) ? "noopener noreferrer" : ""});
  ...
  function buildSocialCard({title, description, url, thumb, platform, tag}){
    ...
    if (thumb){
      const img = el("img", "social-card__img", {src: thumb, alt: ""});
      img.loading = "lazy";
      media.appendChild(img);
    } else {
      media.classList.add("social-card__media--icon");
      const icon = (platform || "").toLowerCase();
      const glyph = icon.includes("linkedin") ? "in" : icon.includes("youtube") ? "▶" : icon.includes("instagram") ? "◎" : icon.includes("x") ? "X" : "★";
      media.innerHTML = `<span aria-hidden="true">${glyph}</span>`;
    }
  ...
  function initCreativeGallery(){
    ...
    const state = { filter:"all", q:"" , videos:[], links:[], library:[], socialHighlights:[]};
...
(function(){
  ...
  // Recruiter Mode toggle (reduce motion + simplify)
  const modeBtn = document.querySelector('[data-mode-toggle]');
  const MODE_KEY = 'jk_portfolio_mode';
  function applyMode(mode){
    const root = document.documentElement;
    if(mode === 'recruiter'){
      root.classList.add('mode--recruiter');
    } else {
      root.classList.remove('mode--recruiter');
    }
    if(modeBtn){
      const label = modeBtn.querySelector('[data-mode-label]');
      const on = mode === 'recruiter';
      modeBtn.setAttribute('aria-pressed', on ? 'true' : 'false');
      modeBtn.setAttribute('aria-label', on ? 'Recruiter mode enabled' : 'Recruiter mode disabled');
      modeBtn.setAttribute('data-mode-state', on ? 'on' : 'off');
      modeBtn.classList.toggle('is-active', on);
      if (label) label.textContent = on ? 'Recruiter Mode: On' : 'Recruiter Mode: Off';
    }
  }
  try{
  ...
  if(modeBtn){
    modeBtn.addEventListener('click', () => {
      const isOn = document.documentElement.classList.contains('mode--recruiter');
      const next = isOn ? 'default' : 'recruiter';
      applyMode(next);
